...
function SHQueryRecycleBin(RootPath: PChar; Query: PSHQueryRBInfo): HRESULT; stdcall; external 'shell32.dll' name 'SHQueryRecycleBinA';
...
var
  ...
  hMainHandle: HWND;
  ulNotifyHandle: ULONG = 0; // Global handle for Shell Notify
  hMutex: THandle;
...
begin
  ...
  if x > (1024) then Result := Format('%d KB', [x div 1024]);
  if x > (1024 * 1024) then Result := Format('%d MB', [x div (1024 * 1024)]);
...
begin
  OutputConsole('RegisterShellNotify -> Registering file system change hooks...');

  // Deregister old notification handle if it exists
  if (ulNotifyHandle <> 0) then begin
    SHChangeNotifyDeregister(ulNotifyHandle);
    ulNotifyHandle := 0;
  end;
  ...
  ulNotifyHandle := SHChangeNotifyRegister(hMainHandle, SHCNF_ACCEPT_INTERRUPTS + SHCNF_ACCEPT_NON_INTERRUPTS, Events, WM_SHELLNOTIFY, Entries, @NotifyEntrys);
end;
// RegisterShellNotify
...
begin
  ...
  Timer.Enabled := False;
  OutputConsole('Timer event -> Updating Recycle Bin Data...');
...
begin
  ...
  if (TMenuItem(Sender).Caption = 'Exit') then begin
    ...
    if (ulNotifyHandle <> 0) then SHChangeNotifyDeregister(ulNotifyHandle);
...
begin
  ...
  if (Msg = WM_SHELLNOTIFY) then begin
    ...
    // SAFETY CHECK: Ensure pointers are not NIL
    if (pItem <> nil) and (pItem^.pidl[0] <> nil) then begin
...
begin
  ...
  Randomize;
  WM_CALLBACK_MESSAGE := WM_USER + 100 + Random(20000);
